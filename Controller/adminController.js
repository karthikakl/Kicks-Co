    ...
    for (const item of order.items) {
      ...
      if (product) {
        ...
        if (sizeIndex !== -1) {
          ...
          console.error(
            `Size not found for product: ${item.productName}, size: ${item.size}`
          );
        }
      }
    }
...
const ChangeOrderStatus = async (req, res) => {
  try {
    ...
    const order = await Order.findOneAndUpdate(
      { _id: orderId },
      { $set: { status: "delivered", "items.$[].ItemStatus": "delivered" } },
      { new: true }
    );
...
const sales = async (req, res) => {
  try {
    ...
    if (filter === "daily") {
      ...
      match = {
        $group: { _id: { $year: "$date" }, total: { $sum: "$total" } },
      };
    } else {
      return res
        .status(400)
        .json({ success: true, message: "filtering not done" });
    }

    const salesData = await Order.aggregate([match, { $sort: { _id: 1 } }]);
